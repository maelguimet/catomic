...
Key unresolved limitations (still current post 2-bj):
...
- big-file tiers/perf: open guardrails + metadata + split harness + manual baselines + line-heavy hotspot smokes + 2026-07-07 open-path phase split + initial persistent "large-file mode" bottom status marker now exist; App open has an explicit content plan (untitled/missing empty vs Small/Large editable full-read vs Huge read-only limited), uses the file::io owned full-read helper for editable opens, avoids the extra LF-only read-buffer clone, LineIndex build uses std newline search for PieceTable opens, App open buffer construction is isolated in app/open.rs, and PieceTable original text is behind OriginalBacking::Owned with no borrowed-slice requirement; Huge present files scan UTF-8/newlines, record ASCII flags plus sparse line checkpoints, and render visible windows with positioned file reads from the same descriptor scanned at open, with descriptor metadata rechecked before ranged reads; edits/save are disabled; status size label is on-disk metadata only; no enforced perf thresholds yet.
...
- Phase 2-au/ax (storage-policy seams): App open buffer construction now lives in app/open.rs, initial LineIndex construction has a direct text path, and PieceTable original text is behind OriginalBacking::Owned without exposing borrowed slices; behavior unchanged for Small/Large editable PieceTable opens.
- Phase 2-ay (limited Huge-file storage, later superseded by 2-bk+): Huge files opened through a read-only LargeFileBuffer that validated UTF-8 and scanned line starts once, then served visible render windows through bounded positioned reads. Edits and save were blocked with explicit messages, and confirmed Ctrl+R reapplied the same size policy. The later paged policy removes the >1-GiB byte-size refusal.
- Phase 2-az (Huge ASCII window optimization): LargeFileBuffer records whether each scanned line is ASCII. Visible windows on ASCII Huge lines now seek directly by byte offset; non-ASCII lines keep scalar-safe window scanning. The ignored sparse 1 GiB smoke now includes a far-horizontal-window render sample. No behavior change or timing gate.
- Phase 2-ba (Huge non-ASCII window checkpointing): LargeFileBuffer now records sparse char-column to byte-offset checkpoints per line during the open scan. Non-ASCII Huge visible windows seek to the nearest prior checkpoint and scan forward from there, preserving scalar column semantics without rereading from the line start for far horizontal windows. Added focused tests plus an ignored 100 MiB dense non-ASCII far-window manual smoke. No editable Huge semantics, mmap, or timing gate.
- Phase 2-bb (Huge descriptor snapshot hardening): LargeFileBuffer now keeps the same file descriptor used for the initial UTF-8/newline scan instead of reopening the path for ranged reads, records fd len/mtime, and fails closed if fd metadata changes before ranged reads. This removes the scan/read reopen race, keeps already-open Huge buffers on the original descriptor after atomic path replacement, and prevents changed same-inode bytes from being rendered as if they matched the scan metadata. Same-size/same-mtime spoofing remains an open limitation.
...
- Phase 2-bj (bounded file-backed PieceTable queries): PieceTable now maps scalar counts, cursor byte offsets, and visible horizontal windows across mixed original/add pieces without materializing whole original lines. The file-backed original retains scan-time ASCII/char/checkpoint metadata; a 400k-scalar Unicode regression test verifies far edited windows stay below a bounded descriptor-read budget. PieceTable's oversized Buffer implementation was split into a focused module. App Huge policy remains unchanged pending the explicit editable-Huge semantics decision.
- Phase 2-bk (accepted paged-file policy + config): files beyond the normal editable policy must page instead of being refused solely by byte size. Pages contain a configurable number of logical lines (`[big_files] page_lines`, default 20,000) loaded from the XDG config path or `~/.config/catomic/config.toml`. Whole-file Ctrl+F must stream across page boundaries with bounded memory. The std-only config parser and startup-owned Plain-safe setting are in place; paged storage/navigation/search wiring follows.
- Phase 2-bl (paged Huge-file buffer foundation): LargeFileBuffer can now retain one configured line page at a time, expose page byte/status metadata, and move forward/backward by rescanning the stable open descriptor. Page changes replace line/checkpoint metadata atomically, reset the page-local cursor, preserve trailing empty-line semantics, and fail closed on descriptor drift. App open/reload policy, terminal navigation, and whole-file Ctrl+F wiring follow.
- Phase 2-bm (paged open/reload policy): Huge and Extreme files now select configurable paged read-only buffers for initial open and confirmed reload. The former fixed >1-GiB refusal is removed; byte size alone no longer rejects a file. Small files remain normal editable opens and Large files remain editable with a warning. Terminal page navigation and whole-file Ctrl+F wiring follow.
- Phase 2-bn (paged terminal navigation): Ctrl+PageDown/Ctrl+PageUp now move through active file pages, reset page-local viewport/cursor state, preserve confirmation tokens, and report first/last page boundaries. The persistent status line includes active page number and descriptor byte range. Whole-file Ctrl+F wiring follows.
- Phase 2-bo (whole-file paged Ctrl+F): Ctrl+F now opens a minimal prompt; editable buffers search their logical lines, while paged files start an explicit cancellable worker over a clone of the stable open descriptor. The worker uses bounded chunks, validates UTF-8, preserves matches across read boundaries, tracks configured page/row/scalar-column positions, and jumps directly to the matching page. Escape cancels prompt or active scan; no search worker exists before explicit invocation. Focused tests cover cross-chunk/Unicode page mapping and a real PTY smoke covers the raw Ctrl+F prompt/result path.

Next intended Phase 2B steps (post 2-bo):
