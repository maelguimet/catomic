...
Key unresolved limitations (still current post 2-ao):
...
- big-file tiers/perf: open guardrails + metadata + split harness + manual baselines + 2026-07-07 open-path phase split + initial persistent "large-file mode" bottom status marker now exist; App open avoids the extra LF-only read-buffer clone; 100 MiB/1 GiB still full read + full materialization (no lazy storage mode); status size label is on-disk metadata only; no thresholds declared or enforced yet.
...
Next intended Phase 2B steps (post 2-ao):
...
- The 2026-07-07 phase split is recorded; piece table construction remains the main measured open/materialization subphase after LF-only and owned-input copy reductions.
