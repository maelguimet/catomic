...
Key unresolved limitations (still current post 2-ay):
...
Next intended Phase 2B steps (post 2-az):
- Decide the next Huge-file policy step: immutable external-change snapshot behavior, editable Huge semantics, or further measured scan/render optimization. Keep any next move narrow and backed by ignored manual measurements.
