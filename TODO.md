...
- Phase 2-ay (limited Huge-file storage): Huge files now open through a read-only LargeFileBuffer that validates UTF-8 and scans line starts once, then serves visible render windows through bounded positioned reads. Edits and save are blocked with explicit messages, and confirmed Ctrl+R reload reapplies the same Large/Huge/Extreme size policy instead of forcing the old full-read path. Ignored manual samples now cover 100 MiB dense, 100 MiB line-heavy, exact-1-GiB sparse open/navigation/render, and >1-GiB refusal. No mmap, rope, editable lazy storage, or immutable external-change snapshot is claimed.
...
Next intended Phase 2B steps (post 2-bl):
