...
Next intended Phase 2B steps (post 2-ar):
...
- Phase 2-ar (narrow full-read helper centralization): file::io::read_to_string now owns the current full-materialization UTF-8 read path using fs::read + String::from_utf8, with focused valid UTF-8 / invalid UTF-8 / missing-file tests. App open and confirmed Ctrl+R reload already route through this helper, and the ignored manual perf harness now measures it directly. Manual 10/100 MiB after-runs stayed in the same shape (100 MiB: read_to_string ~44 ms, PieceTable::from_owned_text ~17 ms, App::new ~61 ms, MaxRSS ~208308 kB), confirming full materialization remains the real limitation. No lazy/mmap/rope/new deps, no thresholds/gates, no watcher signal or save-conflict policy change, Phase 2B not claimed complete.
