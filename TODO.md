...
Key unresolved limitations (still current post 2-ak):
...
Next intended Phase 2B steps (post 2-ak):
...
Repo hygiene note (not a Phase 2B step): global rustfmt baseline applied in a dedicated style commit (pre-existing formatting drift across committed sources resolved; no logic, docs claims, or behavior impact).
