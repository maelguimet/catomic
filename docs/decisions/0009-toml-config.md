...
The standard library has no TOML parser. This dependency is used for startup
settings and for lazily loaded repository/model settings; parsing data does
not construct a linter, repository scanner, process runner, network client, or
other lazy service. Unit tests cover defaults, valid TOML syntax,
...
not require disabling the parser or changing startup service construction.
