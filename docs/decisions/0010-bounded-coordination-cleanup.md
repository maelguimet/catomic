...
| --- | --- | --- |
| `app/mod.rs` | Split. The file mixed state definition, startup construction, the terminal loop, and immutable render projection. `construction.rs`, `runtime.rs`, and `render.rs` now own those activities. Mutually exclusive read-only overlays are grouped in `SurfaceState` because they share absence-at-startup and explicit-invocation lifetime rules. | `mod.rs` is 206 lines; extracted files are 96–177 lines. |
| `app/input.rs` | Split. The long chain encoded a real precedence contract, so it moved to named scope, active-surface, shortcut, editing, and paste routing rather than being divided by arbitrary line ranges. Canonical application shortcuts and ordinary editing now have separate dispatch modules. | The coordination entry is 182 lines; child modules are 41–221 lines. |
| `terminal/render.rs` | Split. Frame composition must finish before terminal transport writes anything. Non-wrapped composition moved to `render/frame.rs`; the root selects a composer, rejects unreasonable dimensions, performs one complete-frame write, and flushes once. Existing styling, ghost text, and wrapped composition remain in their established modules. | The render coordinator is 245 lines and flat composition is 176 lines. |
...
- `buffer/piece_table/buffer_impl.rs` remains one 551-line implementation of the
...
- `app/buffers.rs` remains 453 lines because production ring and active-slot
...
- `file/io.rs` is 354 lines at this base, not the older approximate 470–480-line
...
- Input still consumes normalized `KeyEvent` values and never accesses piece
  table internals.
- The foremost active shortcut scope is selected before translation. Configured
  bindings and explicit unbindings are applied once, then named active surfaces
  precede the inline clanker shortcut, translated surface actions, canonical
  shortcuts, and ordinary editing. Focused tests lock both the named order and
  overlapping surface behavior cases.
- App render projection takes `&self`; terminal render takes `&dyn Buffer`.
  Neither path can mutate editor state. Composition errors and hostile frame
  dimensions are tested to produce zero writer calls, while successful frames
  use one write and one flush.
- SIGWINCH handlers only set an atomic flag. The ordinary event loop owns size
