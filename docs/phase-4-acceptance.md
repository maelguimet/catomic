...
| Markdown preview | `F6` explicitly parses the active Markdown buffer or active large-file page with `pulldown-cmark` into a separate read-only PieceTable. It renders headings, lists, blockquotes, inline/code blocks, tasks, and tables without modifying source history. `F6` or Escape synchronizes the preview's vertical position back to source. |
