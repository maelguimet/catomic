...
- `cargo test --all-targets`: 476 passed, 12 intentional manual tests ignored;
  7 PTY smokes passed.
...
- `cargo test llm::broker::tests`: 5 passed, including active untracked-file
  content drift that leaves Git porcelain unchanged and immutable baselines
  across later retrieval.
