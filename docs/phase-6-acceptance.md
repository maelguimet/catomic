...
| --- | --- |
| Construction gates | Startup owns no pending request, task, client, repo broker, or preview state. `:meow` builds only a draft; Enter constructs the transient worker. Repository preparation is request-local and starts only after an explicit repo-aware command. |
