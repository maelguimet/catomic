...
| Drift refusal | Current-buffer and repo requests pin active-buffer text/path identity while Git/relevant-file state is checked before confirmed send, after the response, and again before apply. The repo pre-send check is worker-polled, so Enter performs no Git capture on the input thread and cannot connect before a successful result. Unit integration covers repo-preparation path drift, pre-send path/disk drift without connecting, post-response path/disk drift, and tracked or untracked changes after preview. |
