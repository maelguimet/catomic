...
| Output validation | Unified-diff parsing checks hunk counts, source context, overlap, bounds, and single-file shape. Selection fallback accepts only one strict JSON string field capped at 64 KiB. Arbitrary prose cannot become an edit. |
...
| Drift refusal | Active-buffer and Git/relevant-file state are checked before confirmed send, after the response, and again before apply. Unit integration changes another tracked file after preview and verifies Enter refuses the edit. |
...
- `cargo test --all-targets`: 462 passed, 12 intentional manual tests ignored;
  7 PTY smokes passed.
- `cargo test app::repo_llm`: 3 passed, including no-connect confirmation and
  repo drift refusal.
- `cargo test app::llm_preview`: 6 passed, including exact one-step undo and
