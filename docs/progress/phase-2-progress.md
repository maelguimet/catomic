...
- Phase 2-bg: LargeFileBuffer's UTF-8/line-metadata scanner moved into `buffer/large_file/scan.rs` behind a private result seam. This returns the main buffer file below the 500-line smell threshold before further storage work while preserving the descriptor, checkpoint, and read-only behavior exactly.
- Phase 2-bh: Buffer gained a streaming `write_to` seam and atomic file I/O gained a counted streaming writer. PieceTable emits piece slices directly, LargeFileBuffer can copy its stable descriptor in bounded chunks, and App save uses the stream without constructing a full logical String. Stream failures preserve the target and clean the temp; Huge save remains policy-disabled pending editable storage.
- Phase 2-bi: PieceTable gained an internal file-backed-original constructor that reuses the Huge UTF-8/newline scanner, retains the scanned descriptor behind original piece ranges, and records original newline offsets so undo/redo index rebuilds do not reread file content. Mixed original/add pieces support edits, history, fallible visible reads, and bounded-chunk streaming output; descriptor metadata drift fails render/save closed. App still selects read-only LargeFileBuffer for Huge files, and PieceTable's edited-line visible query is not yet bounded horizontally.
