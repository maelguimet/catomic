...
///   no saved position exists (imported stdin or a pruned undo save point).
...
pub struct FileState {
    ...
    /// Durable latch for an imported document with no save point or a pruned save point.
    /// Reset only when open/reload/save establishes a new saved position.
    pub(crate) saved_history_unavailable: bool,
...
pub(crate) fn refresh_dirty(file: &mut FileState, buffer: &dyn Buffer) {
    ...
    if !buffer.is_history_position_retained(file.saved_history_position) {
        file.saved_history_unavailable = true;
    }
    file.dirty = file.saved_history_unavailable || pos != file.saved_history_position;
}

/// Imported text has never been saved, including its initial undo position.
pub(crate) fn mark_unsaved(file: &mut FileState) {
    file.saved_history_unavailable = true;
    file.dirty = true;
}
...
pub(crate) fn mark_saved(file: &mut FileState, buffer: &dyn Buffer) {
    ...
    file.saved_history_unavailable = false;
    file.dirty = false;
}
