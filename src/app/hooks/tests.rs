//! Purpose: prove lifecycle hook order, save integration, and failure abort.
//! Owns: deterministic App-level hooks using local shell commands.
//! Must not: send network requests, modify repository files, or bypass command preview.
//! Invariants: hooks are sequential and failure stops the remaining chain.
