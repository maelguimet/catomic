...
use super::super::{
    command_prompt, completion, external_command, help, lint, llm_preview, llm_request, recovery,
    replace, search, view, App,
};
...
const RAW_KEY_PRECEDENCE: [RawKeySurface; 11] = [
    RawKeySurface::Lint,
    RawKeySurface::Help,
...
mod tests {
    ...
    fn precedence_contracts_are_named_and_locked() {
        ...
        assert_eq!(RAW_KEY_PRECEDENCE[5], RawKeySurface::Replace);
        assert_eq!(RAW_KEY_PRECEDENCE[6], RawKeySurface::Search);
        assert_eq!(RAW_KEY_PRECEDENCE[7], RawKeySurface::CommandPrompt);
        assert_eq!(RAW_KEY_PRECEDENCE[10], RawKeySurface::MarkdownPreview);
