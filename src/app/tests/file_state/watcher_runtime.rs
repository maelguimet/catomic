...
// Phase 2-ad: queued watcher signal path for stale pending resolution.
// Changed arms; later disk reverts to a state observed as Unchanged; second
// Changed via watcher clears the stale pending, sets message, returns visible.
#[test]
...
// Optional ignored live OS notify smoke (Phase 2-ad).
// Default cargo test must not depend on live timing; this is manual-only.
// Goal: construct App with a real (non-stub) watcher, external write,
// then a bounded number of check_file_watcher_once_and_render calls
// should eventually produce a visible Modified (pending armed).
#[test]
