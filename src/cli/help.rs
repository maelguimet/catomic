...
pub(super) const UPDATE_OPTIONS: &[UpdateOptionSpec] = &[
    UpdateOptionSpec {
        option: UpdateOption::Stable,
        spellings: &["--stable"],
        description: "Target the newest official stable release (automation default)",
        visibility: Visibility::Public,
    },
    UpdateOptionSpec {
        option: UpdateOption::LatestCommit,
        spellings: &["--latest-commit"],
        description: "Target the exact current commit on official master",
        visibility: Visibility::Public,
    },
    UpdateOptionSpec {
...
pub(super) fn main_help(version: &str) -> String {
    format!(
        concat!(
            ...
            "Usage:\n",
            "  catomic [OPTION] [FILE]\n",
            "  command | catomic [--color=auto|always|never] -\n",
            ...
            "Standard input:\n",
            "  A single FILE argument `-` reads piped or redirected UTF-8 text until EOF\n",
            "  into an unsaved buffer (100 MiB maximum). Ctrl+S opens Save As; Ctrl+Q\n",
            "  confirms discarding the import. Interactive input uses the controlling\n",
            "  terminal and stdout must be a terminal. Without `-`, stdin is not read.\n",
            "  Use `./-` for a file literally named `-`.\n\n",
            "Options:\n",
            "{}\n",
            "Subcommands:\n",
            ...
            "  printf 'hello\\n' | catomic -  Edit piped text, then choose a save path\n",
            ...
            "  catomic update --check       Check stable without changing local state\n\n",
            "Configuration:\n",
...
pub(super) fn update_help() -> String {
    format!(
        concat!(
            ...
            "Options:\n",
            "{}\n",
            "Bare interactive update asks for stable (the default), latest commit, or\n",
            "cancel before network or install actions. Bare --yes and --check select stable\n",
            "deterministically. Explicit target flags never fall back to the other target.\n",
            "--check may contact the official source but does not write.\n",
            ...
            "Official checkout changes are stashed and reapplied around the update.\n\n",
