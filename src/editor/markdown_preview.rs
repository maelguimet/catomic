...
impl PreviewRenderer {
    ...
    fn render_code_block(&mut self, code: &str) {
        ...
        let padding: usize = if available >= 5 {
            2
        } else if available >= 3 {
            1
        } else {
        ...
        let content_width = available.saturating_sub(padding.saturating_mul(2)).max(1);
        ...
        for source_line in code.split('\n') {
            ...
            for line in wrap_code_line(&safe_line, content_width) {
                ...
                let line_width = text_layout::cell_width_from(&line, 0);
                let block = format!(
                    "{}{}{}{}",
                    " ".repeat(padding),
                    line,
...
fn heading_indent(level: HeadingLevel) -> usize {
    match level {
        ...
        HeadingLevel::H3 | HeadingLevel::H4 => 2,
