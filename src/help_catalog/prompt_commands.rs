...
pub(crate) const PROMPT_COMMANDS: &[PromptCommandSpec] = &[
    ...
    command(PromptCommand::Autocomplete, &["autocomplete"], "autocomplete on|off", &[], "Confirm or disable bounded active-buffer inline suggestions; no request starts before confirmation."),
    command(PromptCommand::RunClanker, &["run-clanker", "inline-meow"], "run-clanker", &["inline-meow"], "Discover selection, catblocks, or bounded full-file context, then require confirmation before sending."),
    command(PromptCommand::ClearClankerChanges, &["clear-clanker-changes"], "clear-clanker-changes", &[], "Clear render-only inline clanker change marks without changing document text."),
